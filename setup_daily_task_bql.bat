...
REM that runs run_daily_bql.bat automatically every day.
REM
...
schtasks /create /tn "Airbnb Booking Quality Log" /tr "\"%~dp0run_daily_bql.bat\"" /sc daily /st %RUN_TIME% /f

echo.
echo Scheduled task created: runs run_daily_bql.bat every day at %RUN_TIME%.
